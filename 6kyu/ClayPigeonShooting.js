...
function shoot(x) {
  ...
  for (const round of x) {
    const { P1, P2 } = round[0];
    // console.log('P2: ', P2);
    // console.log('P1: ', P1);
    const isDoublePoints = round[1];

    let peteRoundScore = P1.split('X').length - 1;
    // console.log('peteRoundScore: ', peteRoundScore);
    let philRoundScore = P2.split('X').length - 1;
    //console.log('philRoundScore: ', philRoundScore);

    peteScore += isDoublePoints ? peteRoundScore * 2 : peteRoundScore;
    // console.log('peteScore: ', peteScore);
    philScore += isDoublePoints ? philRoundScore * 2 : philRoundScore;
    // console.log(' philScore: ', philScore);
  }
